...
Bool_t MuDISGenerator::ReadEvent(FairPrimaryGenerator *cpg) {
  ...
  do {
    ...
    if (prob2int > 1.)
      cout << "***WARNING*** MuDISGenerator: prob2int > Maximum density????"
           << prob2int << " maxrho:" << mparam[7]
           << " material: " << mat->GetName() << endl;
  } while (gRandom->Uniform(0., 1.) > prob2int) ;

  // modify weight, by multiplying with average densiy along track??
  cpg->AddTrack(int(mu[0][0]), mu[0][1], mu[0][2], mu[0][3], xmu, ymu, zmu, -1,
                false, mu[0][4], 0., w);
  // in order to have a simulation of possible veto detector hits, let Geant4
  // follow the muon backward
  // due to dE/dx, as soon as the muon travers thick material, this
  // approximation will become bad.
  // a proper implementation however would be to have this better integrated in
  // Geant4, follow the muon, call DIS event, continue
  cpg->AddTrack(int(mu[0][0]), -mu[0][1], -mu[0][2], -mu[0][3], xmu, ymu, zmu,
                0, true, mu[0][4], 0., w);
  // outgoing particles, [did,dpx,dpy,dpz,E], put density along trajectory as
  // weight, g/cm^2
  w = mparam[0] * mparam[4];
  for (int i = 0; i < nf; i++) {
    TVectorD *Part = dynamic_cast<TVectorD *>(dPart->AddrAt(i));
    cpg->AddTrack(int(Part[0][0]), Part[0][1], Part[0][2], Part[0][3], xmu, ymu,
                  zmu, 0, true, Part[0][4], 0., w);
  }
  return kTRUE;
}
// -------------------------------------------------------------------------
